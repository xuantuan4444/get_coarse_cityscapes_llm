...
WEIGHTS_DIR = PROJECT_ROOT / "weights_unet_city_llm_v1"
BEST_CKPT = WEIGHTS_DIR / "unet_city_llm_v1_best.pth"
LAST_CKPT = WEIGHTS_DIR / "unet_city_llm_v1_last.pth"
HISTORY_JSON = WEIGHTS_DIR / "unet_city_llm_v1_history.json"
PLOT_PATH = PROJECT_ROOT / "training_curves_unet_city_llm.png"
...
# RGB(3) + NUM_TARGETS coarse = 17ch. No DINOv2 branch at this ablation level (SAM3+UNet only)
# — IN_CHANNELS_BASE feeds the model's first conv directly, no concat step.
...
class SAM3CityscapesDataset(Dataset):
    ...
    def __init__(self, image_paths, coarse_dir, aug=None):
    ...
    def __getitem__(self, idx):
        ...
        # Augment: RGB + masks (label + valid + NUM_TARGETS coarses).
...
def build_datasets(internal_train_images, internal_val_images):
    train_ds = SAM3CityscapesDataset(internal_train_images, COARSE_CACHE_DIR, aug=build_augment_no_hflip())
    val_ds = SAM3CityscapesDataset(internal_val_images, COARSE_CACHE_DIR, aug=None)
...
# ============================================================================
# Model — ResNet-34 + plain UNet decoder (no ASPP, no DINOv2)
# ============================================================================
...
class ResNet34UNetCityscapes(nn.Module):
    """
    ResNet-34 + plain UNet decoder -- SAM3+UNet ablation level (no ASPP, no DINOv2 branch).

    Input:
      - input_base : [B, 3+NUM_TARGETS, H, W]  = RGB(3) + NUM_TARGETS coarse   (H=512, W=1024)

    Bottleneck: a single 3x3 conv projecting the ResNet-34 layer4 output (512ch) down to 256ch --
    replaces ASPP's 4 dilated-conv branches + global-average-pooling fusion, isolating the ASPP
    contribution for the ablation (compare against ResNet34UNetASPPCityscapes in
    train_unetaspp_city_llm.py / train_unetasppdinov2_city_llm.py).
    ...
    def __init__(self, in_channels_base=17, out_channels=15):
        super().__init__()
...
def build_model():
    model = ResNet34UNetCityscapes(
        in_channels_base=IN_CHANNELS_BASE,
        out_channels=NUM_OUT_CLASSES,
    ).to(DEVICE)
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print("ResNet34UNetCityscapes (SAM3+UNet ablation, no ASPP, no DINOv2)")
...
def train_model(model, train_loader, val_loader, optimizer, scheduler, scaler, autocast_ctx):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        ckpt = dict(
            ...
            arch_version="multi_class_softmax_city_unet_llm_v1",
        )
...
def plot_curves(history, best_epoch, best_miou, out_path):
    ...
    axes[1].plot(ep, miou, "s-", label="Refined (UNet)", color="#2ca02c")
    ...
    plt.suptitle(f"UNet Cityscapes (no ASPP, no DINOv2) -- Best Epoch {best_epoch} | Internal-val mIoU {best_miou:.4f}",
                 fontsize=14)
    plt.tight_layout()
...
if __name__ == "__main__":
    try:
        ...
        print("\n[FATAL] train_unet_city_llm.py failed:", file=sys.stderr)
        traceback.print_exc()
        sys.exit(1)
