...
WEIGHTS_DIR = PROJECT_ROOT / "weights_unetaspp_city_llm_v1"
BEST_CKPT = WEIGHTS_DIR / "unetaspp_city_llm_v1_best.pth"
LAST_CKPT = WEIGHTS_DIR / "unetaspp_city_llm_v1_last.pth"
HISTORY_JSON = WEIGHTS_DIR / "unetaspp_city_llm_v1_history.json"
PLOT_PATH = PROJECT_ROOT / "training_curves_unetaspp_city_llm.png"
...
# RGB(3) + NUM_TARGETS coarse = 17ch. No DINOv2 branch at this ablation level (SAM3+UNet+ASPP)
...
class ResNet34UNetASPPCityscapes(nn.Module):
    ...
    Identical to ResNet34UNetASPPCityscapes in train_unetasppdinov2_city_llm.py except the
    DINOv2 branch (dino_compress, dino_feat concat) is removed -- isolates the DINOv2
...
def build_model():
    ...
    print("ResNet34UNetASPPCityscapes (SAM3+UNet+ASPP ablation, no DINOv2)")
...
def train_model(model, train_loader, val_loader, optimizer, scheduler, scaler, autocast_ctx):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        ckpt = dict(
            ...
            arch_version="multi_class_softmax_city_unetaspp_llm_v1",
        )
...
def plot_curves(history, best_epoch, best_miou, out_path):
    ...
    plt.suptitle(f"UNet+ASPP Cityscapes (no DINOv2) -- Best Epoch {best_epoch} | Internal-val mIoU {best_miou:.4f}",
                 fontsize=14)
    plt.tight_layout()
